...
```
String username = request.getParameter("username"); //단일 파라미터 조회 (하나의 이름에 하나의 값만 있을 때 사용해야 함)
Enumeration<String> parameterNames = request.getParameterNames(); //파라미터 이름들 모두 조회
Map<String, String[]> parameterMap = request.getParameterMap(); //파라미터를 Map으로 조회
String[] usernames = request.getParameterValues("username"); //복수 파라미터 조회 (하나의 이름에 값이 여러 개일 때)
```
